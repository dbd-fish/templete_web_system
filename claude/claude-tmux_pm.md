...
- **タスクフォーマット**：
  ...
  - [🔄] R002 | BACKEND | API実装  | 担当:バックエンドエンジニア | 開始 2025/07/07 14:30
  - [✅] R003 | FULLSTACK | DB設計  | 担当:フルスタックエンジニア | 完了 2025/07/07 15:45
  ```
- **タスク状態**：
  - `[ ]` 未着手
  - `[🔄]` 進行中
...
部下から結果報告を受けたら：
1. 修正されたコードや`claude/reports/R###port.md`のレポート内容や/docに更新されたドキュメントを確認して、要件定義.md通りの成果物になっているか確認
...
- **品質保証**: タスクが完了したら既存の単体テスト、Lint、型チェック、CypressテストがPassするか確認する。Passしない場合は修正する。
- **継続改善**: 効率化できる作業パターンを見つけたら手順書に反映
- **修正方針**: エラーに対する修正は一時的にエラーを改善するのではなく、要件を満たしつつエラーを改善する方法を模索してください。これが一般的な設計はかけ離れる場合のみ、方針の確認をとるようにしてください。
- **コーディング**: コメントにタスク番号のようなコーディングとは関係ない文字を残さない。