...
from app.features.feature_auth.auth_service import create_user, decode_password_reset_token, reset_password, reset_password_email, temporary_create_user, verify_email_token
...
async def get_me(current_user: UserResponse = Depends(get_current_user)):
    ...
    Args:
        current_user (UserResponse): 現在認証されているユーザー情報。
    ...
    try:
        logger.info("get_me - success", user_id=current_user.user_id)
        return current_user
    finally:
...
async def login(request: Request, response: Response, form_data: OAuth2PasswordRequestForm = Depends(), db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        access_token = create_access_token(data={"sub": user.email, "client_ip": client_host})
        ...
        response.set_cookie(
            ...
            # TODO: 現状はアクセストークンであるAuth_tokenの有効期限を長めに設定する
...
async def send_reset_password_email_endpoint(SendPasswordResetEmailData: SendPasswordResetEmailData, background_tasks: BackgroundTasks, db: AsyncSession = Depends(get_db)):
    ...
    Args:
        SendPasswordResetEmailData (SendPasswordResetEmailData): パスワードリセット対象のメールアドレス。
        background_tasks (BackgroundTasks): バックグラウンドタスク。
        db (AsyncSession): 非同期データベースセッション。
