...
class SessionMetricsResponse(BaseModel):
    ...
    metrics: dict[str, int] = Field(
        ...,
        description="アクション別カウント",
        examples=[{"token_created": 45, "token_validated": 128, "token_revoked": 12, "all_tokens_revoked": 3}],
    )


class SecurityAnomalyResponse(BaseModel):
    """セキュリティ異常パターンのレスポンスデータを表すモデル。"""

    user_email: str = Field(
        ...,
        description="異常パターンを示したユーザーメールアドレス",
        examples=["user@example.com"],
    )
    action: str = Field(
        ...,
        description="異常なアクション名",
        examples=["login", "refresh", "token_created"],
    )
    count: int = Field(
        ...,
        description="1時間内のアクション実行回数",
        examples=[15, 101, 25],
    )
    threshold: int = Field(
        ...,
    ...
    timestamp: str = Field(
        ...,
        description="異常検知時刻（ISO形式）",
        examples=["2025-07-22T14:30:00+09:00"],
    )
    device_info: dict[str, str] = Field(
        ...,
        description="関連デバイス情報",
        examples=[{"device_id": "device-uuid-12345", "user_agent": "Mozilla/5.0...", "ip_address": "192.168.1.100", "platform": "windows"}],
    )


class SecurityAnomaliesResponse(BaseModel):
    """セキュリティ異常パターン一覧のレスポンスデータを表すモデル。"""
    ...
    anomaly_count: int = Field(
        ...,
        description="検知された異常パターン数",
        examples=[3],
    )
    anomalies: list[SecurityAnomalyResponse] = Field(
        ...,
        description="異常パターンのリスト",
    )
